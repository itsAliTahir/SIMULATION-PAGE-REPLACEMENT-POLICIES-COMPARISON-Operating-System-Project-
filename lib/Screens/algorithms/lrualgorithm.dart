...
class MyLRUAlgorithm extends StatefulWidget {
  const MyLRUAlgorithm({super.key});

  @override
  State<MyLRUAlgorithm> createState() => _MyLRUAlgorithmState();
}

class _MyLRUAlgorithmState extends State<MyLRUAlgorithm> {
  ...
  Widget build(BuildContext context) {
    ...
    void LRUFunction() async {
      if (referenceStringPointer < int.parse(pageController.text)) {
        if (recentInt >= int.parse(frameController.text)) {
          recentInt = 0;
        }
        for (int i = 0; i < int.parse(frameController.text); i++) {
                      ...
                      child: InkWell(
                        onTap: () {
                          if (mytimer.isActive == false &&
                            ...
                            mytimer = Timer.periodic(const Duration(seconds: 2),
                                (timer) {
                              setState(() {});
                              LRUFunction();
                              setState(() {});
